...
if ISLEARNING:
    ...
    if not loaded:
        files = load_images_from_folder(pos_path)
        for f in files:
            im = Image.open('%s/%s' % (pos_path, f)).convert('L').resize((24,24),Image.ANTIALIAS)
            imagevecs.append((numpy.array(im.getdata()),1))

        files = load_images_from_folder(neg_path)
        for f in files:
            im = Image.open('%s/%s' % (neg_path, f)).convert('L').resize((24,24),Image.ANTIALIAS)
...
family = Image.open('../Databases/Temp/test/Mosaic2.png')
