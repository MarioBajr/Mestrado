...
def drawKeyPoints(img, template, skp, tkp, num=-1):
    ...
    if num < 0 or num > maxlen:
        num = maxlen
    for i in range(num):
        print 'angle', tkp[i].angle, skp[i].angle
        print 'octave', tkp[i].octave, skp[i].octave
        print 'response', tkp[i].response, skp[i].response
        print 'size', tkp[i].size, skp[i].size
        print dir(tkp[i])
...
def match():
    ...
    img = cv2.imread('../Databases/Temp/test/im1.png')
    temp = cv2.imread('../Databases/Temp/test/im2.png')
